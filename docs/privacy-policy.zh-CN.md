...
- 扩展偏好设置，例如语言、弹窗尺寸、规则引擎启用状态和存储模式。
...
如果用户明确启用可选 AI 助手并点击 “AI 生成规则”，Flamingo 会把用户的自然语言需求和当前规则上下文发送给用户选择的 AI 提供方。AI 响应仅用于创建默认禁用的规则草稿，供用户审核。
...
可选 AI 助手只会在用户启用并点击 “AI 生成规则” 后发起外部网络请求。这些请求会发送到用户选择的 AI 提供方，并可能包含用户的需求描述、当前规则上下文以及授权该提供方请求所需的 API Key。
